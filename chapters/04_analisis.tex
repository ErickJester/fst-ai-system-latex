...
RNF-02 & Calidad del análisis
       & El tiempo de procesamiento no es el criterio prioritario para el laboratorio.
         El criterio de calidad es el desempeño del clasificador por clase: se considera
       ...
       & F1 $\geq 85$\,\% por clase (nado, inmovilidad, escalamiento) sobre el
...
\begin{itemize}
  \item \textbf{Confianza de detección de cilindros: $\geq 0.70$.}
        ...
        No existe un valor universal: la metodología estándar en la literatura de
  ...
  \item \textbf{F1 mínimo por clase: $\geq 85$\,\%.}
        ...
        El F1 por clase se evaluará sobre el conjunto de prueba de la fase de
        validación en TT2.
...
OpenCV 4.x \cite{bradski2000}
    & Biblioteca estándar para procesamiento de imágenes y video en Python. Incluye
      ...
      trackers de correlación CSRT y KCF (respaldo clásico frente a ByteTrack).
...
ByteTrack \cite{zhang2022_bytetrack}
    ...
    & Tracker principal del pipeline. Con una rata por ROI no se necesita
      ...
      DeepSORT. Se integra a través del modo \texttt{.track()} de Ultralytics con
      \texttt{bytetrack.yaml}.
    & Sí\textsuperscript{*} \\
\hline
Trackers de correlación CSRT \cite{lukezic2017} / KCF \cite{henriques2015}
    & Trackers de correlación basados en filtros discriminativos. CSRT mantiene un
      mapa de confiabilidad espacial (mayor precisión); KCF opera en el espacio de
      características circular (mayor velocidad).
    & Respaldo clásico para cuadros donde ByteTrack no está disponible (sin GPU o
      sin modelo cargado). CSRT es la primera opción; KCF el segundo nivel.
    & No \\
\hline
...
ResNet-50 \cite{he2016}
    ...
    & Se usará si ResNet-18 no alcanza el F1 objetivo en las tres clases. \\
\hline
PyTorch \cite{paszke2019}
    & Framework de aprendizaje profundo para entrenamiento e inferencia de redes
      ...
      learning de ResNet-18 via \texttt{torchvision.models}.
...
scikit-learn \cite{pedregosa2011}
    ...
    & Se usa para calcular métricas de clasificación (F1, precisión y \textit{recall} por clase) y validación cruzada. \\
\hline
...
Dr.\ César Augusto Sandino Reyes López, investigador del Laboratorio de Bioquímica
Estructural, Sección de Posgrado, de la Escuela Nacional de Medicina y Homeopatía (ENMyH-IPN), quien
...
El equipo de desarrollo no interviene en ningún experimento con ratas; eso lo hace
