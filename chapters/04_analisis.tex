...
RF-04 & Autenticación
      & El sistema debe gestionar dos roles con permisos distintos: Investigador
        ...
        (mismos permisos que el Investigador, más la capacidad de aprobar registros,
        modificar y desactivar cuentas).
      & Alta & Sistema \\
\hline
...
RF-07 & Gestión de usuarios
      & El sistema debe permitir que un investigador se registre de forma autónoma
        proporcionando nombre, correo institucional y contraseña. La cuenta queda
        inactiva hasta que el Administrador la apruebe explícitamente desde el panel
        de administración. El solicitante recibe una notificación por correo al
        momento del registro y otra cuando su cuenta es aprobada.
      & Alta & Admin \\
\hline

% ── Carga de video ─────────────────────────────────────────
RF-08 & Carga de video
      & El sistema debe aceptar únicamente archivos en formato \texttt{.mp4}.
        Cualquier otro formato debe ser rechazado con un mensaje de error específico
        antes de intentar subirlo al servidor.
      & Alta & Sistema \\
\hline
RF-09 & Carga de video
      & El sistema debe permitir subir uno o dos videos por experimento: Día~1
        ...
        son opcionales individualmente. El pipeline analiza el video de Día~2
        completo; del video de Día~1 analiza únicamente los primeros 5~minutos y
        solo cuando corresponde al grupo control (ver RN-03).
      & Alta & Investigador \\
\hline
...
RF-11 & Carga de video
      & Al crear un experimento el investigador debe registrar: nombre del
        experimento, fecha, número de ratas a analizar (típicamente~4, pero puede
        ser entre~1 y~4 según el diseño experimental), tratamiento o condición
        experimental, y notas adicionales opcionales.
      & Alta & Investigador \\
\hline
...
RF-13 & Análisis conductual
      & El pipeline de análisis debe ejecutar cuatro etapas secuenciales:
        preprocesamiento del video (decodificación y corrección de contraste),
        detección de cilindros, seguimiento de ratas (\textit{tracking}) y
        clasificación de conductas. Cada etapa debe tener un indicador de estado
        visible en la pantalla de progreso.
      & Alta & Sistema \\
\hline
...
RF-16 & Análisis conductual
      & Si el análisis falla, el sistema debe mostrar el código de error, la
        descripción del problema y un botón para descargar el reporte de diagnóstico
        en PDF. El investigador no puede reiniciar el análisis desde esta pantalla
        (ver RN-04).
      & Alta & Investigador \\
\hline
RF-17 & Análisis conductual
      & El sistema debe clasificar cuadro a cuadro la conducta de cada rata: nado
        activo, inmovilidad o escalamiento \cite{armario2021}. El buceo se clasifica
        como nado activo. Las conductas son mutuamente excluyentes (ver RN-07).
      & Alta & Sistema \\
      \hline
RF-18 & Análisis conductual
      & El sistema debe contabilizar una conducta como episodio válido únicamente si
        se mantiene de forma continua durante al menos 3~segundos consecutivos. Las
        conductas con duración inferior a ese umbral deben ignorarse. Este criterio
        reproduce el estándar del laboratorio en el análisis manual.
      & Alta & Sistema \\
\hline
...
RF-19 & Resultados
      & La pantalla de resultados debe mostrar el tiempo total en segundos y el
        porcentaje de cada conducta por rata y por sesión, en una tabla
        descargable.
      & Alta & Investigador \\
\hline
RF-20 & Resultados
      & El sistema debe ofrecer un desglose de conductas por minuto para cada rata.
      & Media & Investigador \\
\hline
RF-21 & Resultados
...
RF-22 & Resultados
      & El investigador debe poder descargar los datos de resultados en formato CSV
        ...
        \texttt{Escalamiento} e \texttt{Inmovilidad}, con una fila por minuto
...
\hline
RF-31 & Resultados
      & El sistema debe calcular estadísticas de grupo a partir de los experimentos
        que comparten el mismo tratamiento: media, desviación estándar y varianza
        del tiempo total de cada conducta entre las ratas del grupo. Estas
        estadísticas deben incluirse en el reporte descargable (CSV/XLSX) junto con
        los datos individuales por rata.
      & Media & Investigador \\
\hline
...
\begin{itemize}
  \item \textbf{Confianza de detección de cilindros: $\geq 0.70$.}
        En modelos de detección basados en YOLO, la confianza es un hiperparámetro
        que controla el equilibrio entre precisión y \textit{recall}: valores más
        altos aumentan la precisión pero reducen el número de detecciones válidas;
        valores más bajos capturan más objetos a costa de más falsos positivos.
        No existe un valor universal: la metodología estándar en estudios de
        análisis conductual automatizado consiste en trazar la curva
        precisión-\textit{recall} sobre un conjunto de validación específico del
        experimento y seleccionar el umbral que maximiza el F1 en ese dominio
        concreto \cite{stadler2024_yolov8,chan2025_yolo,della_valle2025}. El valor
        $0.70$ es provisional: se obtuvo de las pruebas preliminares de detección
        realizadas con los videos del laboratorio (véase la
        sección~\ref{sec:fact_tecnologica}) y será calibrado formalmente mediante
        ese análisis de curva precisión-\textit{recall} sobre el conjunto de
        validación real durante la fase de implementación en TT2.
  ...
  \item \textbf{Duración mínima de episodio: 3~segundos.}
        Criterio declarado por el investigador colaborador en la entrevista de
        levantamiento. En el análisis manual, una conducta se contabiliza únicamente
        si se mantiene de forma continua alrededor de 3 a 5 segundos; el sistema
        adopta el límite inferior como umbral conservador (ver RF-30).
...
\hline
RN-07 & En cada cuadro del video, a cada rata se le asigna exactamente una
        conducta: nado activo, inmovilidad o escalamiento. No pueden coexistir dos
        conductas al mismo tiempo para la misma rata.
...
RN-08 & Todos los investigadores con cuenta activa pueden consultar los experimentos
        y resultados de cualquier otro investigador del laboratorio. El Administrador
        puede además modificar y desactivar cuentas.
      & Definido en entrevista de levantamiento. Ver RF-04, RNF-04, RF-29. \\
\hline
...
\hline
RN-11 & Si la confianza de detección de los cuatro cilindros no alcanza $0.70$,
        el pipeline se detiene y el sistema genera automáticamente un reporte de
        diagnóstico descargable que indica el motivo del fallo. Los videos que
        superen ese umbral continúan el análisis de forma normal.
      & Evita que el sistema produzca resultados incorrectos sin advertir al
        investigador. Véase la justificación del valor en la
        sección~\ref{sec:umbrales}. Ver RF-14, RF-16. \\
\hline
...
\hline
RN-13 & Si el clasificador no alcanza F1 $\geq 85$\,\% distinguiendo
        las tres conductas por separado, el nado activo y el escalamiento
        se reportan juntos como \textit{conducta activa}. El reporte
        generado indica explícitamente qué nivel de clasificación se
        aplicó, de modo que el investigador pueda interpretar los
        resultados en consecuencia.
      & Contemplado en el alcance (cap.~1) y en el riesgo R-08.
        Complementa RN-11 para el caso en que el pipeline completa
        la detección pero el clasificador no alcanza el F1 objetivo.
        Véase la justificación del valor en la sección~\ref{sec:umbrales}.
        Ver RF-17, RF-18. \\
\hline
...
OpenCV 4.x \cite{bradski2000}
    & Biblioteca estándar para procesamiento de imágenes y video en Python. Incluye
      sustracción de fondo, umbralización de Otsu, morfología, contornos y el
      tracker de correlación CSRT (respaldo clásico frente a ByteTrack).
    & Es la herramienta de referencia para este tipo de trabajo y es compatible con
      YOLOv8 y PyTorch sin configuración extra.
    & No \\
\hline
Ultralytics YOLOv8 \cite{jocher2023}
    ...
    & Se entrenará con imágenes del laboratorio; su viabilidad técnica para
      fondos consistentes está respaldada por estudios previos \cite{stadler2024_yolov8}.
    & Sí \\
\hline
...
ByteTrack \cite{zhang2022_bytetrack}
    & Algoritmo MOT basado en filtro de Kalman y asociación por IoU. Asocia
      detecciones de alta y baja confianza en dos pasadas para mantener trayectorias
      durante oclusiones. No requiere descriptor de reidentificación por apariencia.
    & Tracker principal del pipeline. Con una rata por ROI no se necesita
      reidentificación, por lo que la simplicidad de ByteTrack es una ventaja sobre
      DeepSORT. 
    & Sí\textsuperscript{*} \\
\hline
Tracker de correlación CSRT \cite{lukezic2017}
    & Tracker de correlación discriminativo que mantiene un mapa de confiabilidad
      espacial ponderando las regiones más discriminativas del objeto.
    & Respaldo clásico para cuadros donde ByteTrack no está disponible (sin GPU o
      sin modelo cargado). Si CSRT falla, se congela la última posición conocida.
    & No \\
\hline

\end{longtable}
{\footnotesize\textsuperscript{*}GPU recomendada para inferencia en tiempo real;
el modo CPU es funcional pero más lento.}
...
ResNet-50 \cite{he2016}
    ...
    & Se usará si ResNet-18 no alcanza el F1 objetivo en las tres conductas. \\
\hline
PyTorch \cite{paszke2019}
    & Framework de aprendizaje profundo para entrenamiento e inferencia de redes
      neuronales. Backbone nativo de Ultralytics YOLOv8 y compatible con transfer
      learning de ResNet-18.
    & Es el framework nativo de YOLOv8 (Ultralytics) y el ecosistema de referencia
      para visión por computadora en Python. Compatible con Docker para el despliegue. \\
\hline
scikit-learn \cite{pedregosa2011}
    & Biblioteca de ML para métricas y validación cruzada.
    & Se usa para calcular métricas de clasificación (F1, precisión y \textit{recall} por conducta) y validación cruzada. \\
\hline
...
R-01 & \cellcolor{red!12}\textcolor{red!70!black}{\Large$\bullet$}~\textbf{Alto}
     & Videos con reflejos, iluminación variable o especímenes de pelaje claro que
       hacen fallar el pipeline (confianza de detección $< 0.70$, ver RN-11).
     & Definir con el laboratorio los requisitos mínimos de calidad de video.
       El sistema aplica corrección CLAHE (\textit{Contrast Limited Adaptive
       Histogram Equalization}) \cite{bradski2000} como primer intento automático
       de mejora de contraste; si la confianza sigue por debajo de 0.70 tras CLAHE,
       se genera el reporte de diagnóstico (ver RN-11). \\
\hline
R-02 & \cellcolor{red!12}\textcolor{red!70!black}{\Large$\bullet$}~\textbf{Alto}
     & El dataset de cuadros etiquetados no alcanza para entrenar un clasificador
       con el F1 objetivo.
     & Ampliar el dataset anotando más cuadros con BORIS
       (\textit{Behavioral Observation Research Interactive Software}) y aplicar
       \textit{data augmentation}. Si el escalamiento no se distingue del nado
       con la precisión objetivo, ambas conductas se reportan juntas como
       ``conducta activa'' (ver alcance, cap.~1). \\
\hline
...
R-08 & \cellcolor{orange!15}\textcolor{orange!80!black}{\Large$\bullet$}~\textbf{Medio}
     ...
     & Usar en el entrenamiento únicamente clips inequívocos de escalamiento
       ...
       alcanza la precisión objetivo, reportar ambas conductas juntas como
       ``conducta activa'' (ver alcance, cap.~1). \\
\hline
...
R-06 & \cellcolor{green!12}\textcolor{green!55!black}{\Large$\bullet$}~\textbf{Bajo}
     ...
     & El sistema genera alertas al 80\,\% de uso de disco y ejecuta limpieza
       automática al 90\,\% (ver RF-28, RNF-09). \\
\hline
...
grabación reales del laboratorio: vista lateral, cuatro cilindros simultáneos, cámara de
...
OpenCV, PyTorch) de la plataforma web (Flask, React, PostgreSQL) mediante una
...
El laboratorio realiza entre 3 y 5 experimentos de nado forzado (FST) por semestre,
cada uno con entre 6 y 8 especímenes divididos en grupos (control, referencia y
tratamiento experimental). Cada experimento genera entre 6 y 8 videos de 5~min en el
segundo día del protocolo. El análisis de esos videos es el cuello de botella actual:
anotarlos manualmente ocupa entre 1.5 y 2~h cada uno, lo que convierte el análisis en
...
Los videos grabados con el teléfono o cámara actual son suficientes, siempre que el
...
\begin{itemize}
  ...
  \item El reporte generado debe incluir, al menos, la duración acumulada por conducta
        (nado activo, inmovilidad y escalamiento) desglosada por minuto, de modo que
        sea directamente comparable con la tabla que el investigador construye hoy a
        mano.
  \item Cuando la calidad del video impida una detección confiable (umbral de confianza
        por debajo de 0.70 según RN-11), el sistema debe notificarlo explícitamente en
        el reporte en lugar de entregar un resultado silencioso potencialmente erróneo.
\end{itemize}

Estos tres criterios se verificarán durante la etapa de validación en TT-II con los
videos y el juicio experto del Dr.\ Sandino como referencia.

\subsection{Factibilidad económica}

La factibilidad económica determina si el proyecto puede concluirse dentro de los
recursos con que cuenta el equipo sin necesidad de financiamiento externo. Dado que
todas las herramientas del \textit{stack} son de código abierto y el laboratorio ya
cuenta con equipo de cómputo y cámara, la inversión se concentra en gastos menores
de documentación y, de forma contingente, en servicios de cómputo en la nube durante
...
Equipo de cómputo y cámara de grabación
    & \$0 --- el Laboratorio de Bioquímica Estructural (ENMyH-IPN) pone a disposición
      el equipo existente. \\
\hline
Cómputo acelerado por GPU (entrenamiento del clasificador)
    & \$0 --- se empleará Google Colab en su capa gratuita, que ofrece sesiones de
      hasta 12~h con GPU T4. \\
\hline
Herramientas de desarrollo (Python, Flask, React, PostgreSQL, Docker, OpenCV,
    ...
    & \$0 --- distribución bajo licencias de código abierto sin costo para uso
      académico. \\
\hline
Impresión y encuadernación del documento recepcional
    & \$1{,}200 \\
\hline
Trámites y gestiones administrativas
    & \$400 \\
\hline
\textbf{Subtotal costos fijos} & \textbf{\$1{,}600} \\
\hline
...
Los costos variables están condicionados al avance del proyecto y solo se incurre en
ellos si determinadas situaciones se materializan:
...
Cómputo en la nube (plan de pago en Colab, solo si el dataset supera la
      capacidad del nivel gratuito)
    & \$0--\$600 \\
\hline
Insumos de soporte audiovisual (almacenamiento externo, cables)
    & \$600 \\
\hline
Formación técnica complementaria (materiales de estudio, acceso a plataformas
      de cursos de visión por computadora)
    & \$800 \\
\hline
\textbf{Subtotal costos variables} & \textbf{\$2{,}000} \\
\hline
...
\hline
\textbf{Costo total estimado} & \textbf{\$3{,}600} \\
\hline

\end{longtable}

Con un costo total estimado de \$3{,}600~MXN, la factibilidad económica del proyecto
queda demostrada. El laboratorio recupera esa inversión con la reducción de carga de
trabajo: anotar manualmente un video de 5~min toma entre 1.5 y 2~h; con más de
32~videos por semestre, eso supone más de 200~horas-persona que el sistema elimina.
Al no requerir compra de hardware ni contratar servicios pagos de forma obligatoria,
el prototipo puede desarrollarse con los recursos del equipo académico sin depender de
convocatorias de financiamiento externas.
...
que almacena son coordenadas de seguimiento, duraciones de conducta y metadatos del
...
Los procedimientos experimentales con ratas son realizados íntegramente por el personal
...
manejo de animales en investigación biomédica.
...
PSF (Python), BSD-3-Clause (Flask), MIT (React), PostgreSQL License (PostgreSQL),
Apache~2.0 (Docker y OpenCV), AGPL-3.0 sin fines de lucro (YOLOv8) y GNU GPL~v3
(BORIS \cite{friard2016}). El código fuente, los modelos entrenados y la documentación
