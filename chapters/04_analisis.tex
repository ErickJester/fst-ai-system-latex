...
\begin{itemize}
  \item \textbf{Confianza de detección de cilindros: $\geq 0.70$.}
        Valor provisional obtenido de las pruebas preliminares de detección
        realizadas con los videos del laboratorio (véase la
        sección~\ref{sec:fact_tecnologica}). No se extrae directamente de la
        literatura sobre YOLOv8 \cite{stadler2024_yolov8}, cuyo contexto de
        validación es detección de objetos en general. Se ajustará con los
        videos reales durante TT2.

  \item \textbf{F1 mínimo por clase: $\geq 85$\,\%.}
        Umbral derivado de la variabilidad aceptada entre analistas humanos, declarada
        por el investigador colaborador (Dr.\ César Augusto Sandino Reyes López) en la
        entrevista de levantamiento. El objetivo del sistema es alcanzar un desempeño
        por clase equivalente o superior a ese nivel de consistencia.
...
RN-11 & La detección de cilindros requiere una confianza mínima de 0.70. Si no se
        ...
        provisional obtenido de las pruebas preliminares realizadas con los videos
        del laboratorio (véase la sección~\ref{sec:fact_tecnologica}); no se
        deriva de \cite{stadler2024_yolov8}, cuyo contexto corresponde a detección
        de objetos en general y no a la detección específica de cilindros en el
        FST. Este valor se ajustará durante la fase de validación en TT2 (véase la
        justificación de umbrales en la sección~\ref{sec:umbrales}).
...
\hline
RN-13 & Si durante la fase de validación el clasificador no alcanza
        el F1 objetivo distinguiendo las tres conductas por separado,
        el nado activo y el escalamiento se reportan juntos como
        \textit{conducta activa}. El reporte generado indica
        explícitamente qué nivel de clasificación se aplicó, de modo
        que el investigador sepa interpretar los resultados en
        consecuencia.
      & Contemplado en el alcance (cap.~1) y en el riesgo R-08.
        Complementa RN-11 para el caso en que el pipeline completa
        la detección pero el clasificador no alcanza la precisión
        objetivo. Ver RF-17, RF-18. \\
\hline
