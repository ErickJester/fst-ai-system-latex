...
\begin{longtable}{@{} p{4.3cm} p{11.3cm} @{}}
  ...
  & (\textit{Swimming}.) Movimientos horizontales del cuerpo y las extremidades
    que desplazan a la rata por el cilindro. Se asocia con la actividad del
    sistema serotoninérgico y se incrementa con antidepresivos como la
    fluoxetina. \\[5pt]
  ...
  & Relativo al sistema de neurotransmisión mediado por dopamina. Fármacos que
    ...
    en el FST, aunque su efecto es menos selectivo que el de la fluoxetina sobre el
  ...
  & Media armónica entre precisión y recall: $F_1 = 2 \cdot \frac{P \cdot R}{P + R}$.
    ...
    negativos; se reporta por conducta (inmovilidad, nado activo, escalamiento)
  ...
  & (Estándar de referencia.) Conjunto de etiquetas considerado como la verdad
    absoluta para la evaluación de un sistema automático. Generalmente lo constituyen
    las anotaciones manuales realizadas por un experto, que sirven como referencia
    para medir el desempeño del modelo. \\[5pt]
  ...
  & (\textit{Immobility}.) Conducta en el FST caracterizada por la ausencia de
    movimientos propulsivos; el espécimen flota con movimientos mínimos para
  ...
  % ── J ──────────────────────────────────────────────────────────────
  \textbf{JWT}
  & (\textit{JSON Web Token}.) Estándar abierto para la transmisión
  ...
  & (\textit{Mean Absolute Error} / Error Absoluto Medio.) Métrica de error de
    ...
    Mide la diferencia promedio entre valores predichos y valores de referencia,
    expresada en las mismas unidades que la variable medida. \\[5pt]
  ...
  & Véase \textbf{FST}. Protocolo de dos sesiones (día 1: 15 min de nado de
    ...
    video los tiempos de inmovilidad, nado activo y escalamiento de cada rata. \\[5pt]
  ...
  & (\textit{Region of Interest} / Región de interés.) Subregión rectangular del
    fotograma que delimita el área de análisis de un espécimen específico. En el
  ...
  & Relativo al sistema de neurotransmisión mediado por serotonina (5-HT). Los
    antidepresivos serotoninérgicos como la fluoxetina incrementan específicamente el tiempo
